...
class FarmActivityExplorer extends Page
{
    use UsesPermissionPageAccess;

    protected static ?string $pagePermission = 'view farm activity explorer';

    protected static ?string $navigationIcon = 'heroicon-o-chart-bar-square';
    protected static ?string $navigationGroup = 'Farm Intelligence';
    protected static ?string $navigationLabel = 'Farm Activity Explorer';
    protected static ?string $title = 'Farm Activity Explorer';
    protected static ?int $navigationSort = 10;
    protected static ?string $slug = 'farm-activity-explorer';

    protected static string $view = 'filament.pages.accounting.farm-activity-explorer';
    ...
    {
        $this->from = $this->from ?: now('Africa/Nairobi')->startOfMonth()->toDateString();
        $this->to = $this->to ?: now('Africa/Nairobi')->toDateString();
    }
    ...
    {
        return url('/farm-activity-explorer/pdf?' . http_build_query([
            'from' => $this->from,
            'to' => $this->to,
        ]));
    }
    ...
    public function getSnapshot(): array
    {
        if (! Schema::hasTable('animals')) {
            return $this->emptySnapshot('The animals table was not found.');
        }

        $from = $this->cleanDate($this->from, now('Africa/Nairobi')->startOfMonth()->toDateString());
        $to = $this->cleanDate($this->to, now('Africa/Nairobi')->toDateString());
        ...
        $kpis = [
            ['label' => 'Total Animals', 'value' => $total, 'note' => 'All animal records created on or before the selected as-at date.', 'tone' => 'primary'],
            ['label' => 'Active / Current', 'value' => $active, 'note' => $activeRate . '% of the herd is currently productive or available.', 'tone' => 'success'],
            ['label' => 'Dead', 'value' => $dead, 'note' => $mortalityRate . '% mortality indicator within the visible herd records.', 'tone' => 'danger'],
            ['label' => 'Culled', 'value' => $culled, 'note' => 'Animals removed from production due to culling decisions.', 'tone' => 'warning'],
            ['label' => 'Sold', 'value' => $sold, 'note' => 'Commercial exits captured in the animal status records.', 'tone' => 'info'],
            ['label' => 'New Additions', 'value' => $additions, 'note' => 'Animals created within the selected reporting period.', 'tone' => 'accent'],
        ];
        ...
        return [
            ...
            'events' => $events,
            'insights' => $this->buildInsights($total, $statusTotals, $additions, $events, $breedRows, $additionRate),
            'empty_reason' => null,
        ];
    }

    protected function emptySnapshot(?string $reason = null): array
    {
        return [
            'generated_at' => now('Africa/Nairobi'),
            'from' => $this->from,
            'to' => $this->to,
            'period_label' => 'No period available',
            'as_at_label' => 'N/A',
            'total' => 0,
            'kpis' => [],
            'status_rows' => [],
