...
class AccountingSourcePostingResource extends Resource
{
    ...
    {
        return auth()->user()?->can('view accounting source postings') ?? false;
    }
    ...
    public static function table(Table $table): Table
    {
        return $table
            ...
            ->columns([
                ...
                Tables\Columns\TextColumn::make('status')->badge()->color(fn (string $state): string => match ($state) { 'posted' => 'success', 'reversed', 'failed' => 'danger', 'draft' => 'gray', default => 'warning' }),
            ...
            ->actions([
                Tables\Actions\Action::make('openJournal')
                    ->label('Journal')->icon('heroicon-o-arrow-top-right-on-square')
                    ->url(fn (AccountingSourcePosting $record): string => $record->journal_entry_id ? AccountingJournalEntryResource::getUrl('view', ['record' => $record->journal_entry_id]) : '#')
                    ->visible(fn (AccountingSourcePosting $record): bool => filled($record->journal_entry_id) && (auth()->user()?->can('inspect accounting source postings') ?? false)),
            ])
            ->bulkActions([
                Tables\Actions\BulkAction::make('exportSelected')
                    ->label('Export Selected')->icon('heroicon-o-arrow-down-tray')->color('gray')
                    ->visible(fn (): bool => auth()->user()?->can('export accounting source postings') ?? false)
                    ->action(fn (Collection $records) => app(AccountingBulkExportService::class)->csv($records, [
                        'Posting Key' => 'posting_key', 'Source Type' => 'source_type', 'Source ID' => 'source_id',
                        'Action' => 'source_action', 'Reference' => 'source_reference',
                        'Journal' => fn ($record) => $record->journalEntry?->journal_number,
                        'Status' => 'status', 'Attempts' => 'attempts',
                        'Posted At' => fn ($record) => $record->posted_at?->format('Y-m-d H:i:s'), 'Error' => 'last_error',
                    ], 'source-postings-' . now()->format('Ymd_His') . '.csv')),
            ]);
    }

    public static function getPages(): array
    {
        return ['index' => Pages\ListAccountingSourcePostings::route('/')];
    }
}
